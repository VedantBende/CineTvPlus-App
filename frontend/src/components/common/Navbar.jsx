...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-gradient-to-b from-netflix-black to-transparent">
      <div className="container-custom">
        <div className="flex items-center justify-between h-14 sm:h-16 md:h-20">
          ...
          <div className="hidden md:flex items-center justify-center space-x-4 lg:space-x-8 lg:w-2/4">
            <Link to="/" className={`transition flex items-center space-x-1.5 text-sm lg:text-base font-medium ${isActive('/') ? 'text-white' : 'text-gray-400 hover:text-white'}`}>
            ...
            <Link to="/movies" className={`transition flex items-center space-x-1.5 text-sm lg:text-base font-medium ${isActive('/movies') ? 'text-white' : 'text-gray-400 hover:text-white'}`}>
            ...
            <Link to="/tv" className={`transition flex items-center space-x-1.5 text-sm lg:text-base font-medium ${isActive('/tv') ? 'text-white' : 'text-gray-400 hover:text-white'}`}>
            ...
            {isSignedIn && (
              <Link to="/favorites" className={`transition flex items-center space-x-1.5 text-sm lg:text-base font-medium ${isActive('/favorites') ? 'text-white' : 'text-gray-400 hover:text-white'}`}>
            ...
            <div className={`relative flex items-center justify-end h-10 transition-all duration-300 ease-in-out ${isSearchOpen ? 'w-[200px] xs:w-[240px] sm:w-[280px] md:w-[300px] mr-2' : 'w-8 sm:w-10'}`}>
              ...
              >
                <input
                  ...
                  placeholder="Search..."
                  className="bg-gray-800 bg-opacity-90 text-white px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-2 rounded-l border-0 focus:outline-none focus:ring-5 focus:ring-netflix-red w-32 min-w-40 sm:w-48 md:w-64 flex-1 text-sm backdrop-blur-sm"
                ...
                <button
                  ...
                  className="ml-1 sm:ml-2 text-gray-400 hover:text-white flex items-center justify-center touch-target p-1 shrink-0"
                  tabIndex={isSearchOpen ? 0 : -1}
              ...
              <button
                ...
                className={`absolute right-0 text-white hover:text-gray-300 transition p-1.5 sm:p-2 flex items-center justify-center touch-target ${
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-black bg-opacity-98 border-t border-gray-800 backdrop-blur-md animate-slide-down">
            <div className="py-2 max-h-[calc(100vh-3.5rem)] overflow-y-auto">
              <Link 
                to="/" 
                className={`block py-3 px-4 transition text-base font-medium touch-target border-l-2 ${isActive('/') ? 'text-accent-red bg-accent-red/10 border-accent-red' : 'text-white hover:text-gray-300 hover:bg-gray-800 border-transparent'}`} 
              ...
              <Link 
                to="/movies" 
                className={`block py-3 px-4 transition text-base font-medium touch-target border-l-2 ${isActive('/movies') ? 'text-accent-red bg-accent-red/10 border-accent-red' : 'text-white hover:text-gray-300 hover:bg-gray-800 border-transparent'}`} 
                onClick={closeMobileMenu}
              >
                Movies
              </Link>
              <Link 
                to="/tv" 
                className={`block py-3 px-4 transition text-base font-medium touch-target border-l-2 ${isActive('/tv') ? 'text-accent-red bg-accent-red/10 border-accent-red' : 'text-white hover:text-gray-300 hover:bg-gray-800 border-transparent'}`} 
                onClick={closeMobileMenu}
              >
                TV Shows
              </Link>
              {isSignedIn ? (
                <>
                  <Link 
                    to="/favorites" 
                    className={`block py-3 px-4 transition text-base font-medium touch-target border-l-2 ${isActive('/favorites') ? 'text-accent-red bg-accent-red/10 border-accent-red' : 'text-white hover:text-gray-300 hover:bg-gray-800 border-transparent'}`} 
                    onClick={closeMobileMenu}
                  >
                    My List
                  </Link>
